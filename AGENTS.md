...
- 系统用户可见中文文案一律使用简体中文；XLSX 系统预设表头只保留千牛与小红书实际导出的简体字段，其他格式由使用者通过自定义别名维护。
- 系统所有源码、模板、HTTP/Email/PDF 内容与附件文件名编码一律使用 UTF-8；不得依赖平台默认编码。
- 不要在主機上個別啟動前端或後端開發服務器；若需要運行中系統協助驗證，使用 `docker compose -f docker-compose.dev.yml up --build` 啟動完整開發環境。正式單一 JAR／容器回歸才使用根目錄 `docker-compose.yml`，不需請使用者手動啟動。
