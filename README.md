...
- 手动新增提供订单编号、买家实付金额、应发货时间、备注标签、买家留言、商家备注与订单付款时间；前三项以红色 `*` 标示并在前后端校验。
- 手动新增的备注标签套用字段识别设置中的加急文字规则；`P` 加数字的订单编号判定为小红书，其余判定为千牛。重复订单编号会被拒绝，不覆盖原工单。
...
- 两个字段一起保存到固定 ID `1` 的 `app_setting`。
- 表单相对原值发生变化时就启用保存；提交时才验证金额与时间格式，Enter 也不能绕过验证。

### 9.2 字段识别设置

- 通过 `GET/PUT /api/settings/import-fields` 读取与保存完整设置快照。
- 七个固定字段会显示必填/选填状态；每个字段只显示一份「别名」清单，不可删除的预设别名在前，使用者新增且可删除的别名接在后方。
