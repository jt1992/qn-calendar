...
- `sourceCode`：来源设置中的内部稳定 ID；既有 `QIANNIU`、`XIAOHONGSHU` 等值继续保留，新来源由后端产生 `SRC_<UID>`。它不在设置界面中编辑，来源改名也不会改变 ID。
...
- 手动新增与 XLSX 使用相同的期限优先级：先解析商家备注中的发货/收到日期，再解析买家留言，最后才使用必填的应发货时间作为 fallback。
