...
- 订单备注会显示于完工统计表。
- 前端顶部导航列提供全局 Email 入口，不再放在日历工具列；Email Dialog 可选周表、月表或完工统计表。
- 周表 Email 可选日期范围，依日期分组列出订单编号、开始时间、结束时间、工时、发货日期与备注；发货日期以红色醒目显示，备注使用较大字级。
- 周表 Email 若日期范围超过一周，会每 7 天拆成一段，段落之间保留间距并加入打印分页样式。
- 月表 Email 只需选择月份，依日期分组列出订单编号、开始时间、结束时间与工时。
- 完工统计表 Email 以订单月份为基准，选择月份后输出前端完工统计表同字段：订单编号、订单备注、订单价格、原本预估时长、实际总时长、差异时间与时薪。
- 周表与月表 Email 都改为打印友善的紧凑 HTML table，避免旧周历格状版面留下过多空白。
