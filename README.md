...
- 先以原始 XLSX 文件名比对「基础设置」目前的订单来源名称；忽略英文字母大小写，`小紅書` 与 `小红书` 视为同名。命中时保存该来源的识别文字、名称、标签颜色与单字标签。
...
- 订单来源选项默认千牛、小红书，以横跨两栏的 InputTag 新增；点击既有标签或新增完成后，下方可编辑来源名称、唯一识别文字、十六进制标签颜色与单一文字标签，并使用编辑区内的保存按钮提交。重新聚焦「继续添加」会收起编辑表单。保存后会依 `sourceCode` 同步既有待排与日历工单并立即刷新画面。必须保留 1–20 个、名称和识别文字不可重复。
