...
| 资料 | 兼容表头 |
|---|---|
| 订单编号 | `orderno`、`订单编号`、`訂單編號` |
| 价格 | `price`、`orderprice`、`amount`、`buyerpaidamount`、订单价格、买家实付金额、价格、金额及繁体形式 |
| 加急 | `urgent`、`isurgent`、加急、急件、备注标签及繁体形式 |
| 买家留言 | `buyermessage`、`buyerremark`、买家留言及繁体形式 |
| 商家备注 | `merchantremark`、`sellerremark`、商家备注及繁体形式 |
| 订单时间 | 付款/支付/订单/下单时间或日期的中英文、繁简形式 |
| 最晚发货 | latest ship/deadline、应发货时间、最晚发货日期/时间及繁体形式 |
...
设置 Dialog 有三个 tab，默认打开「Email 收件者」；打开状态和当前 tab 通过 URL query 保存。
...
- 620px 以下双栏表单改为单栏。
- tooltip 支持鼠标 hover、键盘 focus、Escape，并会避开 viewport。
