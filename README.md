...
- 真实导出 XLSX 会以 `订单编号` 新增或更新工单，并使用 `买家实付金额` 估算原始工时、`备注标签` 判断加急、`买家留言` 与 `商家备注` 组合订单备注。
- 顶部导航提供全局设置，分为「Email 收件者」、「Email 寄件者」与「基础设置」三个 tab；默认打开「Email 收件者」，刷新页面后会通过 URL query 保留当前设置 tab。
- 全局设置对话框固定对齐主内容区顶部，设置 tab 会完整显示且不产生滚动条；收件者标题区紧接 tab 内容且不重复显示分隔线，切换不同内容高度的设置 tab 时不会整体上下跳动。
- 「基础设置」可调整预估工时基础金额与周表默认开始时间；金额默认每 100 元换算为 1 小时，周表时间默认 `06:00`，保存后写入 SQLite。
- 导入时会以 `order_no` 判断新增或更新；既有工单会更新价格、备注、加急、最晚发货时间、订单时间与预估工时，同时保留待排、已排、暂停与完成状态。纯日期型最晚发货日会转为当天 `23:59:59`。
- 同一份 XLSX 内若有重复订单编号，每列都会解析并以最后一笔有效资料为准；导入结果分别显示新增、更新与错误列，不使用跳过笔数。
- XLSX 上传标题右侧提供可重复使用的问号说明，可用鼠标 hover 或键盘 focus 查看必填、选填与兼容字段名称。
...
- 待排卡片以一般字重完整显示订单编号、`急` 标记、`¥` 价格金额、工时控制器、最晚发货时间，以及右侧垂直置中的删除按钮；清单内不重复显示「待排」与「订单价格」文字。短按待排或日历工单会复制不含 `#` 的订单编号，并在顶部导航栏中央以不影响既有布局的方式显示 3 秒半透明成功提示；拖拽排程不会触发复制，删除前会要求确认。
...
- 周视图载入、切换日期或由月视图切回时，会从当前 7 天内最早的工单时刻开始显示；没有工单时使用全局设置中的「周表默认开始时间」。
...
- 今天尚未完成且已有暂停记录的日历工单仍可整体拖动；移动后只保留暂停与继续时间都位于新片段区间内的记录，超出新区间的暂停记录会删除，以维持实际工时统计正确。
- 今天尚未完成且已有暂停记录的日历工单 resize 时固定开始时间，只能从下边缘把结束时间延后，不可缩短或从上边缘调整。
...
- 今日片段拖出日历范围后放开会立即清空整张工单的全部片段与暂停记录并回到待排，不播放回弹动画。
- 工单从日历移回待排后，待排卡片工时会恢复为原始预估时长。
- 非今日片段拖出日历时只移除该片段；若同一工单仍有其他片段，工单不回到待排，最后一段移出后才回到待排。
...
- 可重新导入原始 XLSX 更新既有订单内容；系统不会建立重复订单，也不会改变原有待排、已排、暂停或完成状态。
- 订单备注会显示于完工统计表。
- 完工统计表的「预估工时」与「工时差」来自工单保存的预估工时；调整全局基础金额后，新导入或重新导入的订单会使用新基础金额。时薪维持按 `订单价格 / 实际工时` 计算。
- 前端顶部导航列提供全局 Email 入口，不再放在日历工具列；Email Dialog 可选周表、月表或完工统计表，顶部位置与全局设置 Dialog 对齐，发送成功提示显示在 Dialog 发送按钮左侧。
- 「Email 收件者」设置可维护常用收件人的姓名与 Email，并显示寄送次数及最近寄送时间；SMTP 确认寄送成功后，系统才会以独立短交易新增或更新收件者，避免邮件已寄出却因 SQLite 锁冲突回传失败。
- 前端表单验证会一次检查当前画面的所有必填字段，并把每项结果持续显示在对应字段名称旁，同时以红色边框标示检核未通过的输入框；输入、失焦与等待都不会清除提示，只有离开当前表单画面或提交成功并重置表单时才会清除。此规则已覆盖全局设置（基础设置、寄件者 SMTP、收件者新增与编辑）、Email 寄送（收件人、主题、周日期与月份）及 XLSX 文件格式检查。
- 全局设置中的保存与确认按钮只会在当前表单相较原设置有有效变化时启用；恢复原值后会再次禁用，Enter 也不能绕过此限制，鼠标移至禁用按钮时会显示禁止光标。
- Email Dialog 的收件人栏使用 InputTag 式多选输入；focus 且尚未输入文字时会列出所有未选取的常用收件人，搜索选项只在同一行显示姓名与 Email，不显示寄送次数，输入姓名或 Email 后可继续筛选，并以鼠标或方向键加 Enter 选取。Tag 优先显示姓名、hover 显示完整 Email，也可直接输入新 Email 后按 Enter 或逗号加入。
...
- 全局错误、成功与状态提示通常会在 5 秒后自动消失；复制订单编号成功提示为 3 秒，重复触发同一提示时会重新显示并重置计时。
...
应用内「全局设置」的「基础设置」tab 可调整预估工时基础金额与周表默认开始时间。两者都保存在 SQLite 中；基础金额默认值为 `100`，代表每 100 元换算为 1 小时预估工时，周表默认开始时间为 `06:00`。如果当前周已有工单，画面会优先从该周最早的工单时刻开始显示。
...
`.env` 可设定服务端端口与业务时区；业务时区默认使用北京时间 `Asia/Shanghai`，并可通过 `APP_TIME_ZONE` 覆盖。SQLite 默认使用用户家目录；若以 jar 直接启动时需要改数据目录，必须在启动进程前设置 `QN_CALENDAR_DATA_DIR` 环境变量：
