...
系统别名会作为初始默认值显示在「全局设置 → 字段设置」，不可删除；使用者可为七个 canonical 字段新增或删除自定义别名。同一字段同时命中一个系统别名和一个自定义别名时，自定义别名优先，因此可用自定义价格栏覆盖预设的 `用户应付金额(元)`；若同时命中两个系统别名或两个自定义别名，整份文件会被拒绝，避免静默选错栏位。
...
- 「字段设置」可新增完全匹配或包含匹配的加急文字；例如 `红旗` 不是预设规则，只有使用者自行加入后才代表加急。
...
设置 Dialog 有四个 tab：Email 收件者、Email 寄件者、基础设置、字段设置。默认打开「Email 收件者」；打开状态和当前 tab 通过 URL query 保存。
...
- 七个固定字段会显示必填/选填状态及不可删除的系统别名；使用者可维护自定义别名。
...
成功状态通常为 200；手动新增收件者为 201；删除待排、删除收件者、发送 Email 为 204。
