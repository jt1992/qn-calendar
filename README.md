...
| `import_urgent_match_rule` | 旧版自定义加急文字；升级后保留空表供兼容 |
| `remark_tag_definition` | 可配置备注标签的稳定身份、名称、颜色与显示顺序 |
| `remark_tag_match_rule` | 各备注标签的自定义包含文字；旧匹配类型栏仅供资料升级兼容 |
| `work_order_remark_tag` | 工单与一个或多个备注标签的关联 |
...
- `sourceCode`：来源设置中的内部稳定 ID；既有 `QIANNIU`、`XIAOHONGSHU` 等值继续保留，新来源由后端产生 `SRC_<UID>`。它不在设置界面中编辑，来源改名也不会改变 ID。有值时它是来源判断的唯一依据，只有 null 或空白时才 fallback 到旧 `source` 栏位；两者都缺失的历史资料按千牛处理。
- `sourceName`、`sourceBadgeColor`、`sourceBadgeText`：工单保存来源显示资料；修改同一 `sourceCode` 的全局来源设置时会同步更新既有工单。
- `remarkTags`：按设置顺序回传工单匹配到的备注标签名称与颜色；系统 `URGENT` 标签同时维持既有 `urgent` 排序语义。
- `actualMinutes`：
...
- 只读取第一个 sheet。
- 工作表第一行必须是表头行，每个字段占一列。
- 空白资料行略过且不计入资料笔次；只有实际进入字段解析的公式 cell 才会计算，空白行检测不会提前计算未使用的 fallback。
  ...
  "errors": [
    {
      "row": 8,
      "recordNumber": 7,
      "message": "应发货时间格式不正确，请使用 yyyy-MM-dd 或 yyyy-MM-dd HH:mm:ss"
    }
  ]
}
```

`errors` 中的 `row` 是包含表头行在内的 Excel 1-based 行号；`recordNumber` 是排除表头行与空白行后，从 1 开始累计的非空资料笔次。前端使用 `recordNumber` 显示「第 N 笔数据的{字段错误}」，例如「第 7 笔数据的应发货时间格式不正确，请使用 yyyy-MM-dd 或 yyyy-MM-dd HH:mm:ss」。

### 3.2 表头对应

表头占工作表第一行，每个标准字段对应一列；资料列顺序不固定。表头会 trim、转小写，并移除空白、`_`、`-`。至少必须出现订单编号、买家实付金额、应发货时间三个 canonical 字段；即使备注中能解析发货日，也不能省略应发货时间列。

| 标准字段 | 兼容表头 |
|---|---|
| 订单编号 | `订单编号`、`订单号` |
| 买家实付金额 | `买家实付金额`、`用户应付金额(元)` |
| 备注标签 | `备注标签`、`包裹备注标记` |
| 买家留言 | `买家留言`、`用户备注` |
| 商家备注 | `商家备注`、`包裹备注信息` |
| 订单付款时间 | `订单付款时间`、`支付时间` |
...
导入错误中的字段名称统一使用表中的标准字段名；金额相关错误显示「买家实付金额」，期限字段相关错误显示「应发货时间」。

系统预设表头只保留千牛与小红书实际导出的简体字段。其他平台、旧格式或繁体表头可由使用者在字段识别设置中新增自定义别名；资料按表头名称读取，不依赖列顺序。

预设别名会作为初始默认值显示在「全局设置 → 字段识别设置」的同一份 InputTag 别名清单前方且不可删除；使用者可在清单尾端输入并按 Enter 或逗号新增别名，自定义别名提供删除按钮且删除前会确认。同一字段同时命中一个预设别名和一个新增别名时，新增别名优先，因此可用新增的买家实付金额列覆盖预设的 `用户应付金额(元)`；若同时命中两个预设别名或两个新增别名，整份文件会被拒绝，避免静默选错列。

订单来源与小红书状态：

- 先以原始 XLSX 文件名比对「基础设置」目前的订单来源名称与单字标签；任一文字被包含即视为命中，忽略英文字母大小写，并将 `小紅書` 正规化为 `小红书`。命中时保存该来源的内部 ID、名称、标签颜色与单字标签。
- 文件名同时命中多个来源时整份拒绝，避免静默选错；完全没有命中时才依订单编号 fallback：`P` 加纯数字为 `XIAOHONGSHU`，其余为 `QIANNIU`。
- 小红书的「买家实付金额」字段预设读取 `用户应付金额(元)`。
...
- 系统只预置「加急」标签，不再预置 `true`、`yes`、`y`、`1`、`是` 或「急件」等旧匹配文字。
- 每个标签都会以标签名称自动进行包含匹配；「字段识别设置」也可为标签加入其他「包含文字」。例如标签名称为「加急」时，`请加急处理` 会命中；「急件」只有在使用者加入为包含文字后才会命中。
- 全部规则都使用忽略英文字母大小写的包含匹配，不再区分完全匹配；`延后`、`不限时间` 只是可自行建立的标签范例，不是预设值。
- 同一列可命中多个标签，回传顺序依标签设置顺序；未知文字不会造成资料行错误。
- 只有内部 `systemKey = URGENT` 的标签影响既有加急排序；「不限时间」等自定义标签不会绕过最晚发货规则。
...
- 同一则备注有多个候选日期时，按出现顺序寻找第一个有效日期；前面的无效候选不会让解析提前降级到下一来源。
- 备注无法解析出任何有效日期时继续下一来源。

解析取得有效期限后立即停止；商家备注成功时不会再解析买家留言或应发货时间列，买家留言成功时也不会再解析应发货时间列。因此，未实际使用的应发货时间单元格即使为空或格式错误，也不会产生逐笔错误；只有真正进入 fallback 时才执行必填与格式验证。
...
有备注标签的工单使用设置顺序第一项的颜色作为外框；Tooltip 依序显示 `[加急][延后]` 等标签名称。没有标签时维持一般外框；旧加急资料升级后仍显示默认红框。
...
- 手动新增的来源由基础设置选项决定；系统保存内部 ID、名称、标签颜色与单字标签，待排与日历共用这份显示资料。备注标签与 XLSX 使用相同的多标签匹配设置；重复订单编号会被拒绝，不覆盖原工单。
- 手动新增与 XLSX 使用相同的期限优先级：先解析商家备注中的发货/收到日期，再解析买家留言，最后才使用应发货时间作为 fallback。现行 UI 仍要求选择应发货时间；后端 API 则只在前两者均未解析出日期时才解析并验证该值，未使用的空值或错误格式不会阻止建立工单。
...
- 订单来源选项默认千牛、小红书，以横跨两栏的 InputTag 新增；界面只编辑来源名称、标签颜色与标签单一文字，不显示或要求数据库识别文字。来源名称 trim 后忽略大小写不可重复；新来源保存时由后端产生 `SRC_<UID>` 内部 ID，改名时 ID 不变。
- 标签单一文字只接受一个中文字或一个英文字母。读取旧设置时，数字、emoji 等无效值会改用来源名称中的第一个中文字或英文字母；名称中也找不到时使用「其」。颜色输入固定显示 `#` 前缀，使用者只输入最多六位 `0-9` / `A-F`；小写会即时转为大写，API 与数据库仍保存完整 `#RRGGBB`。
- 点击既有来源标签可编辑并使用编辑区内的保存按钮提交。保存后会依内部 ID 同步既有待排与日历工单并立即刷新画面；必须保留 1–20 个来源。
- 来源编辑器在宽度足够时将来源名称、标签单一文字、标签颜色与保存按钮保持同一横排；前三个字段等宽，颜色字段使用与文字输入一致的单一外框，保存按钮与输入框等高。
- 订单来源 InputTag 的边框、底色、文字与互动色均使用各来源设置的标签颜色，编辑色码时会即时预览。
- 删除已保存来源前会先读取受影响工单数；存在工单时，确认文案明确说明这些工单与排程记录会永久删除。确认后在单一交易内删除暂停、片段、工单与来源设置；删除提示使用红色，重新读取设置时不会恢复已删来源。影响数与删除查询优先使用 `sourceCode`，只有该值缺失时才使用旧 `source`，避免冲突的旧栏位误删其他来源工单。
...
- 备注标签字段下方使用另一份 InputTag 维护标签定义；只预置不可删除的系统「加急」，可编辑其名称与颜色，并可新增、编辑或删除其他标签。
- 标签名称本身会自动参与包含匹配；每个标签另以「包含文字」InputTag 维护其他文字，不显示系统文字、匹配方式下拉或添加按钮。按 Enter 或逗号加入、以删除按钮移除，最后通过输入框右侧的等高保存按钮与名称和颜色一并保存。
- 标签选项 chip 的边框、底色、文字与互动色都会跟随标签颜色即时变化，避免保留与自定义颜色冲突的固定蓝色。
- 聚焦备注标签的「继续添加」输入框时会收起当前标签编辑器；再次选择既有标签即可重新展开编辑。
- 同一工单可命中多个标签。待排、周表与月表外框使用第一项颜色，hover/focus 依设置顺序显示全部方括号标签名称；只有系统加急标签参与既有加急排序。
- 自定义别名、标签定义与包含文字保存在独立资料表；保存时会检查跨字段别名冲突、标签名称、六位色码、空值、重复与长度。
- 「订单付款时间」只接受付款/支付时间语义；订单时间、下单时间、下单日期的繁简别名不会显示或参与导入，后端会删除数据库中遗留于该字段的同名自定义别名并拒绝重新加入。
- 新增或确认删除别名后会自动保存完整快照；新增、编辑或删除标签以及增删包含文字都只修改本地草稿，并由编辑区内的单一保存按钮一并提交。保存失败则恢复服务器上的原设置；标签改名或改色后立即刷新待排与目前日历，既有工单同步使用新显示资料。
- 字段识别设置不再提供底部保存按钮。
...
| `GET` | `/api/settings/import-fields` | 读取 XLSX 字段与备注标签设置 |
| `PUT` | `/api/settings/import-fields` | 替换 XLSX 自定义字段、标签与匹配规则 |
...
- 启动前解析自订 port 的优先序是 `--server.port`、JVM `server.port` / `SERVER_PORT`、环境变量 `SERVER_PORT`、当前或父目录 `.env`，最后才是默认 8080；取得高优先值后不会载入或转换较低来源，只有前述来源都为空时才读取 `.env`。启动后以实际 `local.server.port` 为准，也不会转换未使用的 `server.port`。
...
- XLSX 表头、可配置别名/多备注标签规则、小红书待配货过滤与来源、日期、去重、重导状态保留。
- 待排排序与工时。
...
- SPA fallback/cache。
- 桌面 URL、nonce 与单一实例。
- 手动新增工单期限 fallback 的 MockMvc API contract。

当前没有：

- 前端 unit/component/E2E 测试。
- 其他 controller 的系统性 API contract 测试。
- 真实 SMTP 自动测试。
