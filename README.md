...
- `source`：`QIANNIU` 或 `XIAOHONGSHU`；升级前既有资料视为 `QIANNIU`。
- `actualMinutes`：
...
- 支持点击选档和拖拽上传；整个上传区域 hover 或键盘聚焦时显示 XLSX 字段说明，文件检核错误显示在上传区域下方。
- 前端先检查文件名副档名 `.xlsx`。
...
| 订单时间 | 付款/支付/订单/下单时间或日期的繁简形式 |
| 最晚发货 | 应发货时间、最晚发货日期/时间及繁体形式、`承诺发货时间` |

资料按表头名称读取，不依赖栏位顺序。

预设别名会作为初始默认值显示在「全局设置 → 字段识别设置」的同一份别名清单前方且不可删除；使用者新增的别名接在后方并可删除。同一字段同时命中一个预设别名和一个新增别名时，新增别名优先，因此可用新增价格栏覆盖预设的 `用户应付金额(元)`；若同时命中两个预设别名或两个新增别名，整份文件会被拒绝，避免静默选错栏位。

小红书来源与状态：

- 有 `小红书编码` 表头，或使用 `订单号` 且订单号符合 `P` 加数字时，来源为 `XIAOHONGSHU`；其他资料为 `QIANNIU`。
- 小红书价格预设读取 `用户应付金额(元)`。
- 小红书只导入 `订单状态 = 待配货`；其他状态不进入工单，计入 `skippedCount`，前端会显示跳过笔数。
- 小红书资料必须能读取 `订单状态`；空白状态是该资料行错误。
- 已存在的订单编号不可改成另一个来源重新导入。
...
- 只有类似 `28号发` 或 `发28号` 时，月份来自同一资料行的订单时间。
- 备注无法解析时继续下一来源。
...
1. 完整 `#订单编号`；卡片右上角的蓝色「千」代表千牛，红色「书」代表小红书。
2. `¥价格` 与工时减/加按钮。
3. 红色时钟最晚发货与删除按钮。
...
- 手动新增的来源由基础设置选项决定；内建千牛、小红书沿用既有标记，自定义来源会保存名称并显示在待排与日历。备注标签套用字段识别设置中的加急文字规则；重复订单编号会被拒绝，不覆盖原工单。
...
设置 Dialog 有四个 tab：Email 收件者、Email 寄件者、基础设置、字段识别设置。默认打开「Email 收件者」；打开状态和当前 tab 通过 URL query 保存。
...
- 订单来源选项默认千牛、小红书，以横跨两栏的 InputTag 新增；必须保留 1–20 个、每项最长 80 字且不可重复，删除前会显示明确确认。
- 三项设置一起保存到固定 ID `1` 的 `app_setting` 与来源选项表。
- 表单相对原值发生变化时就启用保存；提交时才验证金额、时间与来源选项，Enter 也不能绕过验证。

### 9.2 字段识别设置

- 通过 `GET/PUT /api/settings/import-fields` 读取与保存完整设置快照。
- 七个固定字段会显示必填/选填状态；每个字段只显示一份「别名」清单，标准字段名本身不重复显示，不可删除的其他预设别名在前，使用者新增且可删除的别名接在后方。
...
- 新增或确认删除别名、加急文字后会立即自动保存；删除前确认文案会明确指出字段或规则内容，保存失败则恢复服务器上的原设置。
- 字段识别设置不再提供底部保存按钮。
...
- work-order store 的全局错误、成功与状态提示：5 秒。
- 复制订单编号：3 秒。
- 重复触发会重置计时。
- XLSX 逐行错误持续显示。
