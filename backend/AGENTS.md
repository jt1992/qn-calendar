...
|---|---|
| `orderNo` | `订单编号`、`订单号` |
| `price` | `买家实付金额`、`用户应付金额(元)` |
| `urgent` | `备注标签`、`包裹备注标记` |
| `buyerMessage` | `买家留言`、`用户备注` |
| `merchantRemark` | `商家备注`、`包裹备注信息` |
| `paidAt` | `订单付款时间`、`支付时间`；不得把订单/下单时间或日期映射到此字段 |
| `latestShipTime` | `应发货时间`、`承诺发货时间` |

系统别名由 `ImportFieldKey` 提供，只保留千牛与小红书实际导出的简体表头；其他格式由使用者通过自定义别名维护。自定义别名由 `ImportFieldSettingsService#getImportSnapshot()` 合并。同字段同时命中一个系统别名和一个自定义别名时，自定义别名优先；同时命中两个系统别名或两个自定义别名时必须拒绝整份文件。
